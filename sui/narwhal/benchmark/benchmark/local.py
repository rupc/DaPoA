...
class LocalBench:
    ...
    def run(self, debug=False, failpoints=False, release=False):
        ...
        try:
            ...
            for i, addresses in enumerate(workers_addresses):
                for (id, address) in addresses:
                    ...
                    print("Run worker: cmd> "+ cmd)

                    self._background_run(cmd, log_file)

            import os
            NotRunning = os.environ.get('NOT_RUNNING')
            if NotRunning == "true":
                print("Not Running process for test-purpose, i.e., compile and generate keys only.")
                exit(0)
