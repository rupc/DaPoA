...
NODE_BIN="/usr/local/bin/narwhal-node"

echo $(ls /usr/local/bin)

if [ -z "$NODE_BIN" ]; then
    echo "narwhal-node binary is not found!"
    exit 1
fi
set -x
PRIMARY_KEYS_PATH="/validators/validator-$VALIDATOR_ID/primary-key.json"
PRIMARY_NETWORK_KEYS_PATH="/validators/validator-$VALIDATOR_ID/network-key.json"
WORKER_KEYS_PATH="/validators/validator-$VALIDATOR_ID/worker-key.json"
COMMITTEE_PATH="/validators/committee.json"
WORKERS_PATH="/validators/workers.json"
PARAMETERS_PATH="/validators/parameters.json"
DATA_PATH="/validators"

echo $PRIMARY_KEYS_PATH
echo $PRIMARY_NETWORK_KEYS_PATH
echo $WORKER_KEYS_PATH
echo $COMMITTEE_PATH
echo $WORKERS_PATH
